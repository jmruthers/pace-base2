...
- **`PagePermissionGuard`** gates both routes using `pageName="activities"` and `operation="read"`.
...
1. Outer **`PagePermissionGuard`** with `pageName="activities"` and `operation="read"` wraps the main content.
