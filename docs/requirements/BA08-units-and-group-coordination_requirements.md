...
- Ownership notes:
  - Backend: Owns unit, role, and preference persistence/permission contracts.
  - Frontend: Owns organiser unit coordination and preference submission UI boundaries.

## Filename convention

Feature requirement slices use numbered BASE prefixes (`BA##`) and a scope slug:

**`BA08-units-and-group-coordination_requirements.md`**

## Overview

BA08 owns the event-scoped units and group coordination surface. It covers `/units` and `/unit-preferences`, with the shared authenticated BASE shell used for both routes under the shared layout contract.
...
- What changes: Splits unit management and ranked preference submission into explicit bounded workflows.
- What stays: Event-scoped coordination and permission boundaries remain explicit and contract-driven.
...
- Ranking rules for this slice:
  - ranks are positive integers starting at `1`
  - ranks must be unique per `(unit_id, submission batch)`
  - no gaps are allowed in a submitted set (`1..N` contiguous)
  - duplicate session selections in the same submitted set are invalid
  - after submit (`submitted_at` set), edits are blocked in MVP
...
- Unit/role management criteria -> `/units` hierarchy and role workflows -> Unit CRUD and role assignment tests.
- Preference workflow criteria -> `/unit-preferences` ranked draft/submit contracts -> Ranking, draft, submit, and post-submit lock tests.
- Permission criteria -> Shared RBAC guard + mutation permission enforcement -> Denied write and denied submission tests.

## Manual QA pack requirements

- Scenarios: Execute verification flows for unit CRUD, role type updates, preference ranking/reorder, submit, and post-submit read-only state.
- Expected outcomes: Units/roles/preferences remain separate workflows with correct event scope and permission behavior.
...
- Stop on blockers: unresolved preference finality semantics, missing role permission mapping, or inconsistent event-scoped unit contracts.

## Done criteria

- Tests pass: Unit CRUD, role assignment, preference draft/submit, and permission tests pass.
- QA passed: Manual QA evidence for verification scenarios is captured.
- Docs updated: BA08 remains aligned with architecture and coordination boundaries.
