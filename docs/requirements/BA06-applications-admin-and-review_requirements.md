...
- Ownership notes:
  - Backend: Owns application/check read models, status mutation contracts, and token reissue behavior.
  - Frontend: Owns organiser queue/detail/review UX in `/applications`.

## Filename convention

Feature requirement slices use numbered BASE prefixes (`BA##`) and a scope slug:

**`BA06-applications-admin-and-review_requirements.md`**
...
- What changes: Establishes organiser review queue/detail with check-level visibility and approved MVP operations.
- What stays: TEAM queue UX remains out of scope; pass/fail/waive step mutation remains out of MVP without backend contracts.
...
- Allowed mutation boundary for this slice:
  - allowed in MVP: application-level `approve` or `reject`, plus token request resend/reissue for pending guardian/referee checks
  - not allowed in MVP: direct pass/fail/waive mutation of individual checks unless a dedicated backend contract is explicitly added and linked in this slice
...
- Queue/filter/detail criteria -> `/applications` list/detail/review-step implementation -> Queue filter and detail rendering tests.
- Allowed operations criteria -> Backend-owned status actions + token reissue contract -> Approve/reject and reissue tests with unresolved checks visibility.
- Permission/scope criteria -> RBAC guard paths and denied states -> Unauthorized access and mutation-denied tests.

## Manual QA pack requirements

- Scenarios: Execute verification flows for queue loading/filtering, check-detail inspection, allowed status updates, token reissue, and denied access.
- Expected outcomes: Review operations follow approved MVP boundaries and preserve visible unresolved checks/context.
...
- Stop on blockers: missing backend status contracts, unresolved token reissue semantics, or unavailable permission mappings for review operations.

## Done criteria

- Tests pass: Queue, detail, approved operations, denied states, and non-exposure tests pass.
- QA passed: Manual QA evidence for verification scenarios is captured.
- Docs updated: BA06 remains aligned with architecture and database authority references.
