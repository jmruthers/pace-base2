...
- Ownership notes:
  - Backend: Owns shared schema/workflow contract semantics and persistence boundaries.
  - Frontend: Owns authoring/runtime consumption of typed forms contract without workflow leakage.

## Filename convention

Feature requirement slices use numbered BASE prefixes (`BA##`) and a scope slug:

**`BA02-shared-forms-platform-contracts_requirements.md`**
...
- What changes: Moves forms to typed workflow-driven contracts with semantic field identity and app-owned orchestration.
- What stays: Shared primitives remain reusable; BASE-specific semantics remain outside shared core primitives.
...
- Treat `@solvera/pace-core` components/hooks as authoritative consuming-app primitives; do not recreate equivalent local form controls, form-state hooks, or auth/RBAC glue when shared surfaces already exist.
...
- Upstream implementation authority for this slice is [`../../../../packages/core/docs/requirements/CR21-workflow-forms-runtime.md`](../../../../packages/core/docs/requirements/CR21-workflow-forms-runtime.md) (**Forms platform architecture (canonical)**).
...
- Any workflow type outside that taxonomy is rejected unless first approved in upstream CR21 and linked in this slice.
...
- Typed workflow and taxonomy criteria -> Shared forms model and workflow metadata contracts -> Contract tests for workflow type coverage.
- Semantic field identity and submission criteria -> `field_key`-based form/response contracts -> Field-key stability and payload-shape tests.
- Multi-app reuse and access-mode criteria -> App/workflow binding + access-mode contracts -> Cross-workflow and permission-path tests.

## Manual QA pack requirements

- Scenarios: Execute verification flows for typed workflow authoring, workflow-bound submission, and response capture across approved workflows.
- Expected outcomes: Shared forms behavior matches workflow contracts without table/column coupling or BASE-specific workflow leakage.
...
- Stop on blockers: unresolved CR21 authority gaps, pending upstream schema decisions, missing access-mode semantics, or failing contract tests.

## Done criteria

- Tests pass: Contract tests for workflow typing, field identity, submissions, and permissions pass.
- QA passed: Manual QA evidence for verification scenarios is captured.
- Docs updated: BA02 and linked upstream forms-runtime authority remain aligned.
...
- Do not duplicate shared form primitives/hooks inside BASE consuming-app code when `@solvera/pace-core` already exports them.
